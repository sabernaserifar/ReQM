...
def write_lammps_data(fileobj, atoms, specorder=None, force_skew=False,
    ...
    elif atom_style == 'pqeq':
        charges = atoms.get_initial_charges()
        molecule = atoms.get_mol_ids() # I added mol_ids to atoms class
        #print (convert(1, "force", "ASE", units))
        for i, (q, r, m) in enumerate(zip(charges, pos, molecule)):
            # Convert position and charge from ASE units to LAMMPS units
            r = convert(r, "distance", "ASE", units)
            q = convert(q, "charge", "ASE", units)
            #print(symbols[i])
            s = species.index(symbols[i]) + 1
            # compare to full, add 3 zeros which are relative shell positions 
            f.write(
                "{0:>6} {1:>3} {2:>3} {3:>5} {4:23.17g} {5:23.17g} {6:23.17g} 0.0 0.0 0.0\n".format(
                    *(i + 1, m, s, q) + tuple(r)
                )
            )
    else:
